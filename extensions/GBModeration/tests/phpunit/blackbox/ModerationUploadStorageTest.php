...
class ModerationUploadStorageTest extends ModerationTestCase
{
         ...
         */
        public function testMigrationFromPerUploaderStashes(
                ...
                $dbw->delete(
                        "user",
                        ["user_name" => ModerationUploadStorage::USERNAME],
                        __METHOD__,
                );
                $dbw->delete(
                        "actor",
                        ["actor_name" => ModerationUploadStorage::USERNAME],
                        __METHOD__,
                );

                foreach (
                        $dbw->select("moderation", "*", "", __METHOD__)
                        as $row
                ) {
                        $dbw->update(
                                "uploadstash",
                ...
                $stashAccessWrapper = TestingAccessWrapper::newFromObject(
                        $stash,
                );

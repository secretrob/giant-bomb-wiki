		...
		*/
        ...
        public static function getFields()
        {
                ...
                if (
                        RequestContext::getMain()
                                ->getUser()
                                ->isAllowed("moderation-checkuser")
                ) {
                        $fields[] = "mod_ip AS ip";
                }
        ...
        public function getHTML()
        {
                ...
                if (!$this->isMove()) {
                        $line .=
                                "(" .
                                $actionLinkRenderer->makeLink("show", $row->id);

                        if ($wgModerationPreviewLink) {
                                $line .=
                                        " | " .
                                        $actionLinkRenderer->makeLink(
                                                "preview",
                                                $row->id,
                                        );
                        }
                ...
                if ($this->isMove()) {
                        ...
                        $page2Link = $linkRenderer->makeLink(
                                $this->getPage2Title(),
                        );

                        $line .= $this->msg("moderation-move")
                                ->rawParams($pageLink, $page2Link)
                                ->plain();
                } else {
                ...
                $line .= $this->timestampTools->format(
                        $row->timestamp,
                        $this->context,
                );
                ...
                $line .=
                        " " .
                        ModerationCompatTools::commentBlock(
                                $row->comment,
                                $title,
                        );
                ...
                if (!$row->merged_revid) {
                        ...
                        if ($row->conflict) {
                                ...
                                if (
                                        ...
                                        $line .= $this->msg(
                                                "moderation-no-merge-link-not-automoderated",
                                        )->plain();
                                }
                        } else {
                                if (
                                        !$row->rejected ||
                                        $this->timestampTools->canReapproveRejected(
                                                $row->timestamp,
                                        )
                                ) {
                                        $line .= $actionLinkRenderer->makeLink(

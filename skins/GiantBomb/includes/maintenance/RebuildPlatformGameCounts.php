...
class RebuildPlatformGameCounts extends Maintenance
{
    public function __construct()
    {
        ...
        $this->addOption(
            "platform",
            "Specific platform name to rebuild (optional)",
            false,
            true,
        );
        $this->addOption(
            "batch-size",
            "Number of platforms to process at once",
            false,
            true,
        );
        $this->addOption(
            "sleep",
            "Seconds to sleep between batches to reduce load",
            false,
            true,
        );
    }
    ...
    private function ensureCacheTableExists()
    {
        ...
        if ($dbw->tableExists("platform_game_counts", __METHOD__)) {
            $this->output(
                "✓ Cache table 'platform_game_counts' already exists\n",
            );
            return;
        }
        ...
        try {
            ...
            $this->fatalError(
                "Failed to create cache table: " . $e->getMessage(),
            );
        }
    }
    ...
    function rebuildPlatformGameCountCache($platformNames = [])
    {
        ...
        if (empty($platformNames)) {
            ...
            try {
                ...
                $result = $api
                    ->getResult()
                    ->getResultData(null, ["Strip" => "all"]);

                if (isset($result["query"]["results"])) {
                    foreach (
                        $result["query"]["results"]
                        as $pageName => $pageData
                    ) {
                        $platformNames[] = str_replace(
                            "Platforms/",
                            "",
                            $pageName,
                        );
                ...
                error_log(
                    "Error fetching platforms for cache rebuild: " .
                        $e->getMessage(),
                );
                $stats["errors"]++;
                return $stats;
        ...
        foreach ($platformNames as $platformName) {
            ...
            try {
                ...
                $stats["updated"]++;
                error_log(
                    "✓ Updated game count for $platformName: $gameCount games",
                );
            } catch (Exception $e) {
                error_log(
                    "✗ Error updating game count for $platformName: " .
                        $e->getMessage(),
                );
                $stats["errors"]++;
            }
